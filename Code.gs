...
function buildDashboardData_() {
  ...
  for (let r=1; r<values.length; r++) {
    ...
    if (!projects[projectId].stageMap[stageNumber]) {
      projects[projectId].stageMap[stageNumber]=createEmptyStage_(stageNumber);
    }
...
function finalizeProject_(project) {
  ...
  return {
    projectId:project.projectId, projectName:project.projectId, category:'Project',
    currentStage:currentStage,currentStageName:stages[currentStage-1].stageName,
    progress:progress,progressPct:progress,completedStages:completedStages.length,
    totalStages:CONFIG.TOTAL_STAGES,stages:stages
  };
}
...
function parseDocuments_(names,links,statuses,remarks){
  const a=splitCell_(names),b=splitCell_(links),c=splitCell_(statuses),d=splitCell_(remarks),max=Math.max(a.length,b.length,c.length,d.length),out=[];
  for(let i=0;i<max;i++){const name=a[i]||'',link=b[i]||'';if(!name&&!link)continue;out.push({name:name,link:link,status:normalizeDocumentStatus_(c[i]||''),remarks:d[i]||''});}
  return out;
}
function splitCell_(v){const t=clean_(v);return t?t.split(/\r?\n/).map(x=>x.trim()).filter(Boolean):[];}
function normalizeStatus_(v){const x=clean_(v).toLowerCase();if(['completed','complete','closed','done','approved'].includes(x))return'Completed';if(['in progress','in-progress','inprogress','ongoing','started'].includes(x))return'In Progress';if(['blocked','block'].includes(x))return'Blocked';if(['on hold','on-hold','hold'].includes(x))return'On Hold';return'Not Started';}
function strongerStatus_(oldStatus,newStatus){const rank={'Not Started':1,'On Hold':2,'In Progress':3,'Blocked':4,'Completed':5};return(rank[newStatus]||1)>(rank[oldStatus]||1)?newStatus:oldStatus;}
function normalizeDocumentStatus_(v){return clean_(v)||'Not Uploaded';}
function formatDate_(v){if(!v)return'';if(Object.prototype.toString.call(v)==='[object Date]'){if(isNaN(v.getTime()))return'';return Utilities.formatDate(v,CONFIG.TIMEZONE,'dd/MM/yyyy');}return clean_(v);}
...
function mergeText_(oldValue,newValue){const incoming=clean_(newValue);if(!incoming)return oldValue||'';if(!oldValue)return incoming;const p=oldValue.split(/\s*;\s*/).filter(Boolean);if(!p.includes(incoming))p.push(incoming);return p.join('; ');}
function validateHeaders_(headers){const missing=REQUIRED_HEADERS.filter(h=>headers.indexOf(h)===-1);if(missing.length)throw new Error('Tracker is missing these headers: '+missing.join(', '));}
function emptyPayload_(){return{success:true,generatedAt:new Date().toISOString(),source:{spreadsheetId:CONFIG.SPREADSHEET_ID,sheet:CONFIG.SHEET_NAME},totalProjects:0,totalStages:CONFIG.TOTAL_STAGES,projects:[],stages:STAGE_DEFINITIONS};}
function testDashboard(){const d=buildDashboardData_();Logger.log('Projects: '+d.projects.length);d.projects.forEach(p=>Logger.log(p.projectId+' | Stage '+p.currentStage+' | '+p.currentStageName+' | '+p.progress+'%'));}
